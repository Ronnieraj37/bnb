...
export const LLM_READ_TOOLS: ToolSet = {
  ...
  // --- This agent's vertical: Venus health-factor risk analysis ---
  // Purpose-built on top of the generic (and LLM-unsafe) `contract_call_view`
  // so the LLM gets one well-described, scoped tool instead of raw ABI access.
  // Reads are always against bsc-mainnet — real Venus data — regardless of
  // which network this agent's own commerce (negotiate/fund/settle) runs on.
  venus_health_factor: tool({
    description:
      "Read a wallet's real, current Venus Protocol account liquidity on BSC " +
      "mainnet: Comptroller.getAccountLiquidity. Returns liquidity (USD-scale, " +
      "18 decimals) and shortfall — a non-zero shortfall means the account is " +
      "already underwater and eligible for liquidation right now. Use this for " +
      "any task asking about liquidation risk, health factor, or Venus " +
      "collateral safety.",
    inputSchema: z.object({
      wallet_address: z.string().describe("0x wallet address to check on Venus"),
    }),
    execute: async ({ wallet_address }) => {
      // Normalise casing so a user-typed address with inconsistent case never
      // trips viem's strict EIP-55 checksum validation.
      ...
      const raw = (await cr.contractCallView(
        VENUS_COMPTROLLER,
        "getAccountLiquidity(address)",
        [wallet_address],
        ["uint256", "uint256", "uint256"],
        "bsc-mainnet",
      )) as { error?: string; decoded?: [bigint, bigint, bigint] };
      if (raw.error || !raw.decoded) {
        return { source: "Venus Comptroller (bsc-mainnet)", error: raw.error ?? "no data returned" };
      }
      const [errorCode, liquidityRaw, shortfallRaw] = raw.decoded;
      const toUsd = (v: bigint) => Number(v) / 1e18;
      const liquidityUsd = toUsd(liquidityRaw ?? 0n);
      const shortfallUsd = toUsd(shortfallRaw ?? 0n);
      return {
        source: "Venus Comptroller (bsc-mainnet, live read)",
        contract: VENUS_COMPTROLLER,
        error_code: Number(errorCode ?? 0n),
        excess_liquidity_usd: liquidityUsd,
        shortfall_usd: shortfallUsd,
        at_risk_of_liquidation_now: shortfallUsd > 0,
      };
    },
  }),
};
