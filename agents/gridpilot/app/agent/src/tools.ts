...
export const LLM_READ_TOOLS: ToolSet = {
  ...
  // --- This agent's vertical: real price data → grid strategy ---
  // Real recent OHLC from Binance's public spot API — same source the Proven
  // marketplace's own backtester uses. Computed volatility, not a guess.
  price_volatility: tool({
    description:
      "Fetch REAL recent 4h candles for a BSC asset from Binance and compute " +
      "its realised volatility over the window. Use this before proposing any " +
      "grid — never invent a price range or volatility number.",
    inputSchema: z.object({
      asset: z.enum(["BNB", "CAKE", "BTCB", "ETH"]).default("BNB"),
      lookback_candles: z.number().int().min(10).max(500).default(180),
    }),
    execute: async ({ asset, lookback_candles }) => {
      const symbol = { BNB: "BNBUSDT", CAKE: "CAKEUSDT", BTCB: "BTCUSDT", ETH: "ETHUSDT" }[asset];
      const res = await fetch(
        `https://api.binance.com/api/v3/klines?symbol=${symbol}&interval=4h&limit=${lookback_candles}`,
      ...
      const volatilityPct = Math.round(Math.sqrt(variance) * Math.sqrt(6 * 30) * 1000) / 10; // ~30d annualised-ish, 6 bars/day
      return {
        source: "Binance spot klines (live)",
        asset,
        last_price_usd: last,
        window_high_usd: high,
        window_low_usd: low,
        realised_volatility_pct_30d: volatilityPct,
