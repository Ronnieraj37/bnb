...
export function buildRunWork(): RunWork {
  ...
  return async (prompt, { abortSignal }) => {
    ...
    const result = await generateText({
      model,
      system:
        "You are a grid-trading strategy analyst for BNB Chain assets. The " +
        "runtime has already authorized this task through its configured " +
        "commerce rail — complete it now; do not ask for a job ID or " +
        "additional payment. ALWAYS call price_volatility for the real " +
        "current price and volatility before proposing a grid (never invent a " +
        "range). Propose a concrete grid: number of levels, band width around " +
        "the real last price, and reasoning tied to the real realised " +
        "volatility. Warn if volatility looks too low for a grid to be " +
        "worthwhile, or too high for the proposed band. Be concrete and concise.",
      prompt,
