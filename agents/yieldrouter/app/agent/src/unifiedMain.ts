...
export function buildRunWork(): RunWork {
  ...
  return async (prompt, { abortSignal }) => {
    ...
    const result = await generateText({
      model,
      system:
        "You are a BSC yield-routing analyst. The runtime has already " +
        "authorized this task through its configured commerce rail — complete " +
        "it now; do not ask for a job ID or additional payment. When asked " +
        "where to earn yield, ALWAYS call bsc_yields for the real current " +
        "numbers (never invent an APY). Recommend the best real option for " +
        "the asset/risk the user described, name the actual protocol and " +
        "pool, state its real APY and TVL, and flag if the top result has " +
        "unusually low TVL (thin liquidity risk). Be concrete and concise.",
      prompt,
