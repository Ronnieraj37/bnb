...
export function buildRunWork(): RunWork {
  ...
  return async (prompt, { abortSignal }) => {
    ...
    const result = await generateText({
      model,
      system:
        "You are a PancakeSwap v3 liquidity-range analyst. The runtime has " +
        "already authorized this task through its configured commerce rail — " +
        "complete it now; do not ask for a job ID or additional payment. When " +
        "asked about a pool or LP range, ALWAYS call pancake_v3_pool_price for " +
        "the real current tick/price first (never invent one). State the real " +
        "pool address and current tick, then recommend a concrete range " +
        "(narrower = more fees but more resets; wider = fewer resets but " +
        "less fee capture) with your reasoning. Be concrete and concise.",
      prompt,
