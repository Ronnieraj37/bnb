...
export const LLM_READ_TOOLS: ToolSet = {
  ...
  // --- This agent's vertical: real PancakeSwap v3 pool state ---
  // Reads the real PancakeSwap V3 Factory + Pool contracts on bsc-mainnet —
  // real current tick/price, not a guess. Two chained on-chain reads.
  pancake_v3_pool_price: tool({
    description:
      "Look up a real PancakeSwap v3 pool by token pair + fee tier on BSC " +
      "mainnet, and read its REAL current price tick (slot0). Use this before " +
      "recommending any liquidity range or rebalance — never invent a price " +
      "or tick.",
    inputSchema: z.object({
      token_a: z.string().describe("0x address of the first token"),
      token_b: z.string().describe("0x address of the second token"),
      fee_tier: z
        .number()
        .int()
        .default(2500)
        .describe("pool fee in hundredths of a bip, e.g. 100, 500, 2500, 10000"),
    }),
    execute: async ({ token_a, token_b, fee_tier }) => {
      // Normalise casing so user-typed addresses never trip viem's strict
      // EIP-55 checksum validation.
      token_a = token_a.toLowerCase();
      token_b = token_b.toLowerCase();
      const V3_FACTORY = "0x0BFbCF9fa4f9C56B0F40a671Ad40E0805A091865";
      // `contractCallView`'s signature is INPUTS ONLY — no `function` prefix,
      // no `returns (...)` suffix; output decoding is a separate arg.
      const poolRaw = (await cr.contractCallView(
        V3_FACTORY,
        "getPool(address,address,uint24)",
        [token_a, token_b, fee_tier],
        ["address"],
        "bsc-mainnet",
      )) as { error?: string; decoded?: [string] };
      if (poolRaw.error || !poolRaw.decoded) {
        return { source: "PancakeSwap v3 Factory (live)", found: false, error: poolRaw.error };
      }
      const poolAddress = poolRaw.decoded[0];
      if (!poolAddress || /^0x0+$/.test(poolAddress)) {
        return { source: "PancakeSwap v3 Factory (live)", found: false, reason: "no pool at that fee tier" };
      }
      const slot0Raw = (await cr.contractCallView(
        poolAddress,
        "slot0()",
        [],
        ["uint160", "int24", "uint16", "uint16", "uint16", "uint32", "bool"],
        "bsc-mainnet",
      )) as { error?: string; decoded?: [bigint, number, ...unknown[]] };
      if (slot0Raw.error || !slot0Raw.decoded) {
        return { source: "PancakeSwap v3 Pool (live)", found: false, pool_address: poolAddress, error: slot0Raw.error };
      }
      const [sqrtPriceX96, tick] = slot0Raw.decoded;
      // price of token_b in token_a, ignoring decimals (report raw + let the
      // LLM state it qualified, since decimals vary per token).
      ...
      return {
        ...
        current_tick: tick,
        raw_price_ratio: rawPrice,
        note: "raw_price_ratio ignores token decimals — state it as a ratio, not a USD price, unless both tokens are known 18-decimals",
      };
    },
  }),
};
